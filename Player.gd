...
onready var foots = $foot
onready var stamp = $stamp
onready var lelz = $boink
onready var btn = $trigger_button
onready var MainPlayer = $"."
onready var mob = $MobDetector
onready var detector = $"../UserInterface"

onready var musix = "res://MusicPlayer.tscn"
	...
	for index in range(get_slide_count()):
		...
		if collision.collider.is_in_group("mob"):
			...
			if Vector3.UP.dot(collision.normal) > 0.1:
				...
				#For the fucking greedy person LOL
				scale.x += 0.4
				scale.y += 0.4
				scale.z += 0.4
...
func _on_TimerBar_time_to_die():
	...
	mob.hide()
	btn.show()
	get_tree().change_scene("res://cinematic.tscn")
